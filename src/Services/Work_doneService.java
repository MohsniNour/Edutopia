...
public class Work_doneService implements IWork_done {
    ...
    public void add(Work_done work_done) {
        try {
            ...
            pst.setInt(4, work_done.getUploaded_by());
    ...
    public void updateScore(int id, int score) {
        try {
            String query = "UPDATE `work_done` SET `score`=? ,`status`=? WHERE id_activity=?";
            PreparedStatement ps = conn.prepareStatement(query);
            ps.setInt(1, score);
    ...
    public ArrayList<Work_done> getAvailableWorkDoneListByIdActivity(int id_Activity) {
        try {
            String query = "select * from work_done where id_activity=?";
            PreparedStatement ps = conn.prepareStatement(query);
            ps.setInt(1, id_Activity);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                if ("Work_posted".equals(rs.getString("status"))) {
                    Work_done work = new Work_done();
                    work.setId(rs.getInt("id"));
                    work.setUploaded_date(rs.getDate("uploaded_date"));
                    work.setUploaded_by(rs.getInt("uploaded_by"));
                    work.setId_activity(rs.getInt("id_activity"));
                    work.setStatus(rs.getString("status"));
                    work.setWork_file(rs.getString("work_file"));
                    la.add(work);
    ...
    public List<Work_done> getRatedWorkDoneListByIdActivity(int id_Activity) {
        try {
            String query = "select * from work_done where id_activity=?";
            PreparedStatement ps = conn.prepareStatement(query);
            ps.setInt(1, id_Activity);
    ...
    public Work_done FindWorkByIdActivity(int id_Activity) {
        Work_done work = new Work_done();
        try {
            String query = "select * from work_done where id_activity=?";
            PreparedStatement ps = conn.prepareStatement(query);
