 ...
 * @author ADMIN
 */
public class AvailableForumController implements Initializable {
    ...
    private void CourseAction(ActionEvent event) throws IOException {
        Parent root = FXMLLoader.load(getClass().getResource("Home_Course.fxml"));
        Scene scene = new Scene(root);
        Stage window = (Stage) ((Node) event.getSource()).getScene().getWindow();
        window.setScene(scene);
        window.show();
    ...
    private void addAction(ActionEvent event
        ...
        if (txtSubject.getText().isEmpty()) {
            ...
            alert.setTitle("Attention !");
            alert.setHeaderText(null);
            alert.setContentText("Le sujet est vide saisir un sujet ");
            alert.showAndWait();
        } else {
            ForumService fs = new ForumService();
